...
const char * const BITCOIN_CONF_FILENAME = "mrclassic.conf";
const char * const BITCOIN_PID_FILENAME = "mrclassicd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "mrclassic" is a composite category enabling all Mrclassic-related debug output
            if(ptrCategory->count(string("mrclassic"))) {
...
#else
    const char* pszModule = "mrclassic";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MrclassicCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MrclassicCore
    // Mac: ~/Library/Application Support/MrclassicCore
    // Unix: ~/.mrclassiccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MrclassicCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MrclassicCore";
#else
    // Unix
    return pathRet / ".mrclassiccore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty mrclassic.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override mrclassic.conf
